...
def epoch_seconds(index: pd.DatetimeIndex) -> np.ndarray:
    """Epoch seconds from a (UTC) DatetimeIndex, indépendamment de la résolution
    du dtype. pandas ≥2.0 conserve unit='s' comme datetime64[s] (au lieu de
    datetime64[ns]), donc l'ancien `index.view("int64") // 10**9` — qui supposait
    des nanosecondes — renvoyait des epochs quasi nuls et bloquait tous les
    signaux (aucun trade). L'arithmétique Timedelta est robuste à ça."""
    return ((index - pd.Timestamp(0, tz="UTC")) // pd.Timedelta(seconds=1)).to_numpy().astype(np.int64)
...
def prepare_frames(m1: pd.DataFrame, granularities) -> tuple:
    ...
    for gran in granularities:
        ...
        frames[gran] = f
        closes_at[gran] = (epoch_seconds(f.index) + gran).astype(np.int64)
...
def run_backtest(strategy_name: str, m1: pd.DataFrame, balance0: float,
    ...
    m1_epochs = epoch_seconds(m1.index)
    n = len(m1)
    ...
    for i in range(warmup, n - 1):
        ...
        if pos is not None:
            ...
            if pos["trail"] is not None:
                # trailing par paliers: à chaque `step` de gain latent MAX atteint,
                # on verrouille `lock` de profit (le SL se resserre, sans jamais se
                # relâcher). Pas de TP fixe. kind "pnl" → step/lock en $ (via le
                # notionnel) ; kind "dist" → step/lock en distance de prix absolue.
                step, lock = pos["trail"]
                entry, notional = pos["entry"], pos["notional"]
                pnl = pos["trail_kind"] == "pnl"
                if pos["dir"] == "long":
                    pos["peak"] = max(pos["peak"], hi)
                    gain = notional * (pos["peak"] - entry) / entry if pnl else pos["peak"] - entry
                    locked = lock * (gain // step) if gain > 0 else 0.0
                    if locked > 0:
                        pos["sl"] = max(pos["sl"], entry + (locked * entry / notional if pnl else locked))
                    if lo <= pos["sl"]:
                        exit_price, why = pos["sl"], ("TRAIL" if pos["sl"] >= entry else "SL")
                else:
                    pos["peak"] = min(pos["peak"], lo)
                    gain = notional * (entry - pos["peak"]) / entry if pnl else entry - pos["peak"]
                    locked = lock * (gain // step) if gain > 0 else 0.0
                    if locked > 0:
                        pos["sl"] = min(pos["sl"], entry - (locked * entry / notional if pnl else locked))
                    if hi >= pos["sl"]:
                        exit_price, why = pos["sl"], ("TRAIL" if pos["sl"] <= entry else "SL")
            else:
        ...
        pos = {
            ...
            "trail": (sig.trail_step, sig.trail_lock) if trailing else None,
            "trail_kind": sig.trail_kind,
            "peak": entry,
        }
        equity_curve.append((m1.index[i], balance))
...
def summarize(name: str, trades: List[dict], curve, balance0: float, quiet=False) -> dict:
    if not trades:
        ...
        return {"strategy": name, "trades": 0, "winrate_%": 0.0, "profit_factor": 0.0,
                ...
                "commission_$": 0.0, "TP": 0, "SL": 0, "BE": 0, "TR": 0}
    ...
    if not quiet:
        ...
        for s, grp in t.groupby("session"):
            net = grp['net'].sum()
            total += net
            print(f"    {s:<8} {len(grp):>4} trades   net ${net:>9.2f}   "
                  f"winrate {100*len(grp[grp['net']>0])/len(grp):.0f}%")
        print(f"    {'TOTAL':<8} {'':>4} trades   net ${total:>9.2f}")
