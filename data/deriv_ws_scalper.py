...
async def _get_proposal(
    ...
    multiplier: float,
    sl_amount: float,
    tp_amount: Optional[float],
) -> dict:
    limit_order = {"stop_loss": round(sl_amount, 2)}
    if tp_amount is not None:            # None ⇒ pas de TP fixe (sortie par trailing)
...
async def open_trade(
    ...
    multiplier: float,
    sl_amount: float,
    tp_amount: Optional[float],   # None ⇒ pas de TP fixe (sortie gérée par trailing)
    ...
    if tp_amount is not None and "take_profit" in vp:
    ...
    changed = (round(c_stake, 2), round(c_sl, 2)) != (round(stake, 2), round(sl_amount, 2))
    if tp_amount is not None:
        changed = changed or (round(c_tp, 2) != round(tp_amount, 2))
    if changed:
        log.info(
            "Clamped to broker limits: stake %.2f→%.2f  SL %.2f→%.2f  TP %s→%s",
            stake, c_stake, sl_amount, c_sl,
            f"{tp_amount:.2f}" if tp_amount is not None else "—",
            f"{c_tp:.2f}" if c_tp is not None else "—",
        )
    ...
    log.info(
        "✅ OPEN %-5s  stake=$%.2f  x%s  SL=$%.2f  TP=%s  entry=%.2f  contract_id=%s",
        direction.upper(), stake, multiplier, sl_amount,
        f"${tp_amount:.2f}" if tp_amount is not None else "trailing",
        entry_spot or 0.0, contract_id,
    )

    # Commission RÉELLE renvoyée par Deriv → taux effectif du notionnel, à
    # comparer directement au --commission-pct utilisé dans le backtest
    # (0.0024 vs 0.018 change tout pour les stratégies à faible edge/trade).
    if commission > 0 and stake > 0 and multiplier > 0:
        notional = stake * multiplier
        log.info(
            "   ↳ commission Deriv réelle: $%.4f sur notionnel $%.2f  →  taux = %.4f%% du notionnel",
            commission, notional, commission / notional * 100.0,
        )
    else:
        log.info("   ↳ commission Deriv non encore renseignée sur ce contrat (relevée à la clôture).")
...
async def manage_trailing_stops(client: DerivWSClient, positions: Dict[int, dict]) -> None:
    """
    Deux modes de gestion de sortie, choisis par position:

    • Trailing P&L (positions portant une clé "trail", ex: alligator-v2):
      verrouille `lock` $ de P&L par `step` $ de P&L latent MAXIMUM atteint
      (cliquet — ne se relâche jamais), à l'identique du backtester.
      ⚠ Le stop_loss Deriv est un montant de PERTE; verrouiller un GAIN revient
      à demander un stop_loss négatif, ce que le broker peut refuser. En cas de
      refus, update_stop_loss() renvoie False et l'ancien stop est conservé —
      à VALIDER sur compte DÉMO avant de s'y fier en réel.

    • Break-even (toutes les autres stratégies): dès que le prix a bougé de
      ≥ BE_TRIGGER_R × la distance SL du trade en sa faveur, on resserre le
      stop_loss à "commission seule" (scratch). Défini en R ⇒ comportement
      identique pour chaque stratégie, et identique au backtester.
    """
    for contract_id, pos in list(positions.items()):
        ...
        if trail is not None:
            ...
            locked = lock * (pos["peak_profit"] // step)      # $ de P&L à verrouiller
            if locked > pos.get("locked_profit", 0.0):
                if await update_stop_loss(client, contract_id, -locked):
                    pos["locked_profit"] = locked
                    pos["sl_amount"] = -locked
                    log.info("↗  Trailing: $%.2f de P&L verrouillé (contract_id=%s)", locked, contract_id)
            continue
    ...
    try:
        while True:
            try:
                ...
                tp_dist = abs(sig.tp_price - entry_price) if sig.tp_price is not None else 0.0
                ...
                log.info(
                    "📶 SIGNAL %-5s  [%s]  stake=$%.2f  x%s  SL=$%.2f  TP=%s  |  %s",
                    sig.direction.upper(), sig.reason, stake, multiplier, sl_amount,
                    f"${tp_amount:.2f}" if tp_amount is not None else "trailing",
                    guard.status_str(equity),
                )
                ...
                if new_pos:
                    if trailing:
                        new_pos["trail"] = (sig.trail_step, sig.trail_lock)
                        new_pos["peak_profit"] = 0.0
                        new_pos["locked_profit"] = 0.0
